...
namespace TanHoaWater.DAL
{
    class C_KH_XinPhepDD
    {
        ...
        public static DataTable findByHSHT(string shs) {
            ...
            TanHoaDataContext db = new TanHoaDataContext();
            db.Connection.Open();
            string sql = " SELECT ttk.SHS,HOTEN, SONHA + ' ' + DUONG as 'DIACHI', TENPHUONG ";
        ...
        public static void Delete(string madot) { 
            try
            {
                TanHoaDataContext db = new TanHoaDataContext();
                var obj = from dd in db.KH_XINPHEPDAODUONGs where dd.MADOT == madot select dd;
                if (obj != null)
                    db.KH_XINPHEPDAODUONGs.DeleteOnSubmit((KH_XINPHEPDAODUONG)obj);
                db.SubmitChanges();
            }
            catch (Exception ex)
            {
